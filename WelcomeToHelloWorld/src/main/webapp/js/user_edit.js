...
document.getElementById('btn1_h').addEventListener('click', function(event) {
	...
	fetch('user_edit_controller', {
	    ...
	    .then(data => {
	        if (data.success) {
	            // 성공 시 다른 화면으로 이동 (쿼리 파라미터 추가)
	            window.location.href = 'Login.html?message=updateSuccess';
	        } else {
